...
const PlaylistCard = ({ limiter, playlists }) => {
  return (
    <>
      {playlists &&
        playlists.slice(0, limiter).map((playlist, idx) => (
          <div key={idx}>
            <Link to={`/playlists/${playlist.id}`}>
              <div className="w-full cursor-pointer  group bg-neutral-800 hover:bg-neutral-700 flex flex-col gap-6 rounded-md transition duration-500 ">
                <div className="h-full relative bg-[#171717] rounded-md p-4">
                  ...
                  <div className="min-h-[62px] box-border">
                    <p
                      href="#"
                      className="text-slate-50 no-underline text-base font-bold truncate"
                    >
                      {playlist.name}
                    </p>
